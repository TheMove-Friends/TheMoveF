...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate ,InternetStatusIndicable{
    ...
    func observeMessages() {
      //  SwiftSpinner.show("Loading Chat Messages...")
        guard let uid = Auth.auth().currentUser?.uid, let toId = user?.id else {
            return
        }
        
        print("current user \(uid)")
        print("chat user \(toId)")
        
        let userMessagesRef = Database.database().reference().child("user-messages").child(uid).child(toId).queryLimited(toLast: 30)
        userMessagesRef.observe(.childAdded, with: { (snapshot) in
            ...
            messagesRef.observeSingleEvent(of: .value, with: { (snapshot) in
                ...
                print("message dictionary \(dictionary)")
        ...
        do
        {
            if let jsonData = data
            {
                ...
                DispatchQueue.main.async {
                 //   self.alert(message:loginDetails.value(forKey: "err_mode") as! String)
    ...
    override func viewDidLoad() {
        ...
        if let userIds : NSDictionary = UserDefaults.standard.dictionary(forKey: "loginDetails")! as NSDictionary {
            ...
            if let userId : String = userIds.value(forKey:"userid") as? String, let fid = self.user?.fbid!{

                let params = ["userid": Int(userId)!, "friendid" : fid] as [String : Any]

                ApiService.callPost(url: URL.init(string: Endpoints().addChatList)!, params:params as [String : Any],viewcontroller: self, finish: self.self.addFriendsApi)
...
//        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(handleKeyboardWillHide), name: UIKeyboardWillHideNotification, object: nil)
    ...
    fileprivate func setupCell(_ cell: ChatMessageCell, message: Message) {
        ...
        if message.fromId == Auth.auth().currentUser?.uid {
            ...
            //incoming gray
            cell.bubbleView.backgroundColor = UIColor(rgb: 0xF0F0F0)  //UIColor(r: 240, g: 240, b: 240)
    ...
    fileprivate func sendMessageWithProperties(_ properties: [String: Any]) {
        ...
        childRef.updateChildValues(values) { (error, ref) in
            ...
            if  self.inputContainerView.inputTextField.text == nil {
            }
    ...
    func sendnotification(){
        ...
        if let userIds : NSDictionary = UserDefaults.standard.dictionary(forKey: "loginDetails")! as NSDictionary {
            ...
            if let userId : String = userIds.value(forKey:"userid") as? String ,let friendId = user?.fbid!, let userName =  userIds.value(forKey:"username") as? String {
                let params = ["title": userName ,"message":self.inputContainerView.inputTextField.text!,"userid":userId,"friendid":friendId] as [String : Any]
